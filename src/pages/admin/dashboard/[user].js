...
const User = ({ result: { user, questions } }) => {
    ...
    const [managementMarks, setManagementMarks] = useState(user.marks ? user.marks.management : 0);
    const [designMarks, setDesignMarks] = useState(user.marks ? user.marks.design : 0);
    const [videoMarks, setVideoMarks] = useState(user.marks ? user.marks.video : 0);
    ...
    return (
        <div className="flex flex-col justify-center items-center m-6">
            ...
            {user.domainsAttempted.map((dom) => {
                ...
                return <Questions domain={dom.domain} questions={ques} key={dom.domain} />;
            })}
            ...
            <form onSubmit={(e) => submitHandler('tech', e)} className="flex flex-col gap-2 mt-5">
                <CustomInput type="number" value={techMarks} setValue={setTechMarks} label="Marks" />
                <CustomInput type="text" value={techComment} setValue={setTechComment} label="Comments" />
            ...
            <form onSubmit={(e) => submitHandler('management', e)} className="flex flex-col gap-2">
                <CustomInput type="number" value={managementMarks} setValue={setManagementMarks} label="Marks" />
                <CustomInput type="text" value={managementComment} setValue={setManagementComment} label="Comments" />
                <Button type="submit">submit</Button>
            </form>
            ...
            <form onSubmit={(e) => submitHandler('design', e)} className="flex flex-col gap-2">
                <CustomInput type="number" value={designMarks} setValue={setDesignMarks} label="Marks" />
                <CustomInput type="text" value={designComment} setValue={setDesignComment} label="Comments" />
                <Button type="submit">submit</Button>
            </form>
            ...
            <form onSubmit={(e) => submitHandler('video', e)} className="flex flex-col gap-2">
                <CustomInput type="number" value={videoMarks} setValue={setVideoMarks} label="Marks" />
                <CustomInput type="text" value={videoComment} setValue={setVideoComment} label="Comments" />
                <Button type="submit">submit</Button>
            </form>
        </div>
    );
