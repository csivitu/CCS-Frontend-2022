...
const Dashboard = ({ result }) => {
    ...
    useEffect(() => {
        let filtered = result.filter(
            (user) => user.username.includes(query) || user.userId.regNo.includes(query.toUpperCase()),
        );
        if (domain) {
            filtered = filtered.filter((user) => user.domainsAttempted.map((dom) => dom.domain).includes(domain));
        }
        if (round) {
    ...
    return (
        <div className="flex flex-col justify-center items-center min-h-screen">
            ...
            <div className="flex flex-row justify-center items-center my-4">
                <CustomInput label="round" type="text" value={round} setValue={setRound} />
            </div>
            <div className="flex flex-row justify-center items-center my-4">
                <CustomInput label="search" type="text" value={query} setValue={setQuery} />
            </div>
            {users.map((usr) => (
                <div key={usr._id} className="text-2xl py-1 font-semi-bold flex flex-row w-2/3">
                        ...
                        switch (domain.domain) {
                                ...
                                return (
                                    <p
                                        className={`self-end mx-3 ${
                                            usr.isChecking.tech && !usr.checked.tech ? 'text-orange-400' : ''
                            ...
                            case 'design':
                                return (
                                    <p
                                        className={`self-end mx-3 ${
                                            usr.isChecking.design && !usr.checked.design ? 'text-orange-400' : ''
                            ...
                            case 'video':
                                return (
                                    <p
                                        className={`self-end mx-3 ${
                                            usr.isChecking.video && !usr.checked.video ? 'text-orange-400' : ''
