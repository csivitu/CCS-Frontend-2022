...
const registerFormSchema = Joi.object({
    ...
    username: Joi.string()
        ...
        .required()
        .label('Username')
        .regex(new RegExp(constants.usernameRegex))
        .error(new Error('Username should be min 3 and max 20 and not include any spaces')),
    ...
    email: Joi.string()
        .required()
        .regex(new RegExp(constants.vitEmailRegex))
        .error(new Error('Invalid VIT Email Address.')),
    phone: Joi.string()
        .required()
        .label('Phone')
        .custom((value) => {
