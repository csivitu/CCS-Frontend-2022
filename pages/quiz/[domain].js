...
const domain = ({ success, domain, questions, endTime, startMessage }) => {
            ...
            <div className="flex flex-nowrap flex-row justify-center relative">
                {success ?
                    ...
                    <div className="flex flex-col items-center justify-center h-screen">
                        <p className="text-center">You have already attempted!</p>
                        <Link href="/" passHref>
...
export async function getServerSideProps(ctx) {
    ...
    if (!success) {
        return { props: { success: false, domain, startMessage } }
    }

    const { success: questionSuccess, result, message } = await getQuestions({ domain }, cookies);
    console.log(result)
    if (!questionSuccess)
        return { props: { success: false, domain } }
